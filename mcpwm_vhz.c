/*
	Copyright 2019 Alejandro Pina Ortega	a.pina-ortega@ieee.org
...
// Private types
typedef struct {
	float vd_target; // ajpina
	float vq_target; // ajpina
	float max_duty;
	float duty_now;
	float phase;
	float phase_fake; // ajpina
	float i_alpha;
	float i_beta;
	float i_abs;
	float i_abs_filter;
...
void observer_update_vhz(float v_alpha, float v_beta, float i_alpha, float i_beta,
...
static void control_voltage(volatile motor_state_t *state_m, float dt);
...
void mcpwm_vhz_init(volatile mc_configuration *configuration) {
	utils_sys_lock_cnt();
	...
	dmaStreamAllocate(STM32_DMA_STREAM(STM32_DMA_STREAM_ID(2, 4)),
			3,
			(stm32_dmaisr_t)mcpwm_vhz_adc_int_handler,
			(void *)0);
	...
	TIMER_UPDATE_SAMP(MCPWM_VHZ_CURRENT_SAMP_OFFSET);
...
bool mcpwm_vhz_init_done(void) {
	return m_init_done;
}

void mcpwm_vhz_set_configuration(volatile mc_configuration *configuration) {
	m_conf = configuration;
	...
	TIMER_UPDATE_SAMP_TOP(MCPWM_VHZ_CURRENT_SAMP_OFFSET, top);

}

mc_state mcpwm_vhz_get_state(void) {
	return m_state;
}

bool mcpwm_vhz_is_dccal_done(void) {
	return m_dccal_done;
}
 ...
 * Switch off all FETs.
 */
void mcpwm_vhz_stop_pwm(void) {
	mcpwm_vhz_set_current(0.0);
}
 ...
 */
void mcpwm_vhz_set_duty(float dutyCycle) {
 ...
 * The duty cycle to use.
 */
void mcpwm_vhz_set_duty_noramp(float dutyCycle) {
	// TODO: Actually do this without ramping
	mcpwm_vhz_set_duty(dutyCycle);
}
 ...
 */
void mcpwm_vhz_set_pid_pos(float pos) {
 ...
 * The current to use.
 */
void mcpwm_vhz_set_current(float current) {
 ...
 */
void mcpwm_vhz_set_brake_current(float current) {
	if (fabsf(current) < m_conf->cc_min_current) {
		m_control_mode = CONTROL_MODE_NONE;
		m_state = MC_STATE_OFF;
		stop_pwm_hw();
 ...
 */
void mcpwm_vhz_set_handbrake(float current) {
	if (fabsf(current) < m_conf->cc_min_current) {
		m_control_mode = CONTROL_MODE_NONE;
		m_state = MC_STATE_OFF;
		stop_pwm_hw();
 ...
 * The RPM to use.
 */
void mcpwm_vhz_set_openloop(float current, float rpm) {
	if (fabsf(current) < m_conf->cc_min_current) {
		m_control_mode = CONTROL_MODE_NONE;
		m_state = MC_STATE_OFF;
		stop_pwm_hw();
 ...
 */
void mcpwm_vhz_set_openloop_phase(float current, float phase) {
	if (fabsf(current) < m_conf->cc_min_current) {
		m_control_mode = CONTROL_MODE_NONE;
		m_state = MC_STATE_OFF;
		stop_pwm_hw();
 ...
 */
void mcpwm_vhz_set_current_offsets(volatile int curr0_offset,
 ...
 * The RPM to use.
 */
void mcpwm_vhz_set_openloop_duty(float dutyCycle, float rpm) {
 ...
 * The phase to use in degrees, range [0.0 360.0]
 */
void mcpwm_vhz_set_openloop_duty_phase(float dutyCycle, float phase) {
...
float mcpwm_vhz_get_duty_cycle_set(void) {
	return m_duty_cycle_set;
}

float mcpwm_vhz_get_duty_cycle_now(void) {
	return m_motor_state.duty_now;
}

float mcpwm_vhz_get_pid_pos_set(void) {
	return m_pos_pid_set;
}

float mcpwm_vhz_get_pid_pos_now(void) {
 ...
 */
float mcpwm_vhz_get_switching_frequency_now(void) {
 ...
 */
float mcpwm_vhz_get_sampling_frequency_now(void) {
 ...
 * The motor current.
 */
float mcpwm_vhz_get_tot_current(void) {
 ...
 */
float mcpwm_vhz_get_tot_current_filtered(void) {
 ...
 */
float mcpwm_vhz_get_abs_motor_current(void) {
 ...
 */
float mcpwm_vhz_get_abs_motor_current_unbalance(void) {
 ...
 */
float mcpwm_vhz_get_abs_motor_voltage(void) {
 ...
 * The magnitude of the motor current.
 */
float mcpwm_vhz_get_abs_motor_current_filtered(void) {
 ...
 * The motor current.
 */
float mcpwm_vhz_get_tot_current_directional(void) {
 ...
 * The filtered motor current.
 */
float mcpwm_vhz_get_tot_current_directional_filtered(void) {
 ...
 * The input current.
 */
float mcpwm_vhz_get_tot_current_in(void) {
 ...
 */
float mcpwm_vhz_get_tot_current_in_filtered(void) {
 ...
 */
int mcpwm_vhz_get_tachometer_value(bool reset) {
 ...
 * be this number divided by (3 * MOTOR_POLE_NUMBER).
 */
int mcpwm_vhz_get_tachometer_abs_value(bool reset) {
 ...
 */
float mcpwm_vhz_get_phase(void) {
 ...
 * The phase angle in degrees.
 */
float mcpwm_vhz_get_phase_observer(void) {
 ...
 * The phase angle in degrees.
 */
float mcpwm_vhz_get_phase_encoder(void) {
	float angle = m_phase_now_encoder * (180.0 / M_PI);
	utils_norm_angle(&angle);
	return angle;
}

float mcpwm_vhz_get_vd(void) {
	return m_motor_state.vd;
}

float mcpwm_vhz_get_vq(void) {
 ...
 */
void mcpwm_vhz_get_current_offsets(volatile int *curr0_offset, volatile int *curr1_offset, volatile int *curr2_offset) {
 ...
 */
void mcpwm_vhz_encoder_detect(float current, bool print, float *offset, float *ratio, bool *inverted) {
	mc_interface_lock();
 ...
 */
float mcpwm_vhz_measure_resistance(float current, int samples) {
	mc_interface_lock();
 ...
 */
float mcpwm_vhz_measure_inductance(float duty, int samples, float *curr) {
	m_samples.avg_current_tot = 0.0;
	m_samples.avg_voltage_tot = 0.0;
	m_samples.sample_num = 0;
	...
	float t = (float)TIM1->ARR * m_samples.measure_inductance_duty / (float)SYSTEM_CORE_CLOCK -
			(float)(MCPWM_VHZ_INDUCTANCE_SAMPLE_CNT_OFFSET + MCPWM_VHZ_INDUCTANCE_SAMPLE_RISE_COMP) / (float)SYSTEM_CORE_CLOCK;
 ...
 * The average d and q axis inductance in microhenry.
 */
float mcpwm_vhz_measure_inductance_current(float curr_goal, int samples, float *curr) {
	...
	for (float i = 0.02;i < 0.5;i *= 1.5) {
		float i_tmp;
		mcpwm_vhz_measure_inductance(i, 10, &i_tmp);
	...
	float ind = mcpwm_vhz_measure_inductance(duty_last, samples, curr);
 ...
 */
bool mcpwm_vhz_measure_res_ind(float *res, float *ind) {
	const float f_sw_old = m_conf->foc_f_sw;
	...
	for (float i = 2.0;i < (m_conf->l_current_max / 2.0);i *= 1.5) {
		if (i > (1.0 / mcpwm_vhz_measure_resistance(i, 20))) {
			i_last = i;
			break;
		}
	}
	...
	*res = mcpwm_vhz_measure_resistance(i_last, 200);
	*ind = mcpwm_vhz_measure_inductance_current(i_last, 200, 0);
 ...
 */
bool mcpwm_vhz_hall_detect(float current, uint8_t *hall_table) {
	mc_interface_lock();
...
void mcpwm_vhz_print_state(void) {
	...
	commands_printf("Fake Phase:   %.2f", (double)m_motor_state.phase_fake);
	...
	commands_printf("vd_target:    %.2f", (double)m_motor_state.vd_target); // ajpina
	commands_printf("vq_target:    %.2f", (double)m_motor_state.vq_target); // ajpina
...
// ajpina INIT
float mcpwm_vhz_get_voltage_ref(float f_ref){
	float v_ref;
	...
	if( f_ref_abs > f_max ){
		v_ref = v_max;
	} else if( f_ref_abs > f_min && f_ref_abs <= f_max ){
		v_ref = (v_max - v_min)*(f_ref_abs - f_min)/(f_max - f_min) + v_min;
	} else {
		v_ref = v_min;
	}
...
float mcpwm_vhz_voltage_slew_rate_limiter(float v_in){
	float delta_time_in_secs = (float)ST2MS(chVTTimeElapsedSinceX(m_tick_time)) / 1000.0;
	m_tick_time = chVTGetSystemTimeX();
	float rate = ( v_in - m_previous_voltage ) / delta_time_in_secs;
	const float max_volt_per_sec = 0.01;
	float v_out = 0.0;
	if( fabsf(rate) > max_volt_per_sec ){
		v_out = rate * delta_time_in_secs + m_previous_voltage;
	} else {
		v_out = v_in;
	}
	m_previous_voltage = v_out;
	return v_out;
}

// ajpina END

float mcpwm_vhz_get_last_adc_isr_duration(void) {
	return m_last_adc_isr_duration;
}

void mcpwm_vhz_tim_sample_int_handler(void) {
	if (m_init_done) {
...
void mcpwm_vhz_adc_int_handler(void *p, uint32_t flags) {
	(void)p;
	(void)flags;
...
//	float ic = -(ia + ib);
	...
	if (m_samples.measure_inductance_now) {
		...
		const uint32_t samp_time = duty_cnt - MCPWM_VHZ_INDUCTANCE_SAMPLE_CNT_OFFSET;
...
//		m_motor_state.i_beta = ONE_BY_SQRT3 * ib - ONE_BY_SQRT3 * ic;
		...
		//	duty_set = 0.0;
		//}
		// ajpina disabled to allow start
		...
		// Run observer
		if (!m_phase_override) {
			observer_update_vhz(m_motor_state.v_alpha, m_motor_state.v_beta,
			...
			if (!m_phase_override) {
				if (duty_abs < m_conf->foc_sl_d_current_duty) {
					...
					id_set_tmp = utils_map(duty_abs, 0.0, m_conf->foc_sl_d_current_duty,
												fabsf(iq_set_tmp) * m_conf->foc_sl_d_current_factor, 0.0);
					// ajpina END
					// before ajpina
					// id_set_tmp = utils_map(duty_abs, 0.0, m_conf->foc_sl_d_current_duty,
...
static THD_FUNCTION(timer_thread, arg) {
	...
	chRegSetThreadName("mcpwm_vhz timer");
	...
	for(;;) {
		...
		// ajpina INIT
		float openloop_rpm = utils_map(fabsf(m_motor_state.iq),
...
static void control_voltage(volatile motor_state_t *state_m, float dt) {
	...
	state_m->vd_int += Verr_d * (ki * dt);  // ajpina
	state_m->vq_int += Verr_q * (ki * dt);  // ajpina
...
//			(2.0 / 3.0) * max_duty * SQRT3_BY_2 * state_m->v_bus);
	...
	// ajpina DO SOMETHING TO LIMIT THE CURRENT
	...
	const float i_alpha_filter = c * state_m->id_filter - s * state_m->iq_filter;  // ajpina
	const float i_beta_filter = c * state_m->iq_filter + s * state_m->id_filter;   // ajpina
